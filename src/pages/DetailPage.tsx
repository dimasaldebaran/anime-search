...
export default function DetailPage() {
  ...
  useEffect(() => {
    if (!animeId) return
    if (detail) return
    const p: any = dispatch(fetchAnimeDetail(animeId))
    return () => { p?.abort?.() }
  }, [animeId, detail, dispatch])

  if (status === 'loading' && !detail) return <div className="panel"><p className="muted">Loading…</p></div>
  if (error && !detail) return <div className="panel"><p>Error: {error}</p></div>
  ...
  return (
    <div className="panel">
      <div style={{display:'flex', gap:8, alignItems:'center'}}>
        ...
        <FavoriteButton id={detail.mal_id} />
      </div>
